...
const resolves = (u, from) => {
  const clean = u.split('#')[0].split('?')[0];
  if (!clean || clean === '/' || clean === './' || clean === '.') return true;
  const p = clean.startsWith('/') ? path.join(out, decodeURIComponent(clean)) : path.resolve(path.dirname(from), decodeURIComponent(clean));
  if (!p.startsWith(out)) return false;
  return fs.existsSync(p) || fs.existsSync(path.join(p, 'index.html')) || fs.existsSync(p + '.html') || fs.existsSync(p + '/index.html');
};
for (const f of files) {
  const s = fs.readFileSync(f, 'utf8');
  // root-absolute and depth-relative references in attributes
  for (const m of s.matchAll(/(?:href|src|content|<loc>)=?["']?((?:\/|\.\.?\/)[^"'\s<>)\\]*)/g)) {
    const u = m[1];
    if (u.startsWith('//')) continue;
    checked++;
    if (!resolves(u, f)) missing.add(`${path.relative(out, f)} → ${u}`);
  }
  // srcset candidates
  for (const m of s.matchAll(/srcset="([^"]+)"/gi)) for (const cand of m[1].split(',')) { const u = cand.trim().split(/\s+/)[0]; if (!u) continue; checked++; if (!resolves(u, f)) missing.add(`${path.relative(out, f)} → ${u}`); }
