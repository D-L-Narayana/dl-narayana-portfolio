  ...
  return (
    <section id="github" className="section" aria-labelledby="github-title">
      <div className="container">
        ...
        <RevealGroup className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border md:grid-cols-4" as="div">
          <Tile label="Public repositories" value={String(stats.publicRepos)} note={`since ${formatDate(stats.memberSince, { day: false })}`} />
          <Tile label="Commits, public repos" value={nf.format(stats.commits)} note={`${formatDate(github.activity.firstCommit)} → ${formatDate(github.activity.lastCommit)}`} />
          <Tile label="Active weeks" value={`${stats.activeWeeks} / ${stats.weeks}`} note="last 26 weeks" />
          <Tile label="Languages" value={String(langs.length)} note={`${langs[0]?.name} leads at ${langs[0]?.pct.toFixed(0)} %`} />
        </RevealGroup>
        ...
        <div className="mt-16">
          ...
          <RevealGroup as="ul" className="hairline" gap={0.04}>
            {recent.map((r) => (
              <Item key={r.name} as="li" className="grid grid-cols-12 items-baseline gap-x-4 gap-y-1 border-b border-border py-4">
                ...
                <p className="col-span-12 text-sm text-muted sm:col-span-8 md:col-span-6 md:pr-6">{r.description ? r.description.replace(/^[^A-Za-z0-9]+/, '').split(/[.·]\s/)[0] : 'Profile README'}</p>
...
function Tile({ label, value, note }: { label: string; value: string; note?: string }) {
  return (
    <Item className="bg-surface p-6 md:p-8">
