...
await p.goto('http://localhost:8094/',{waitUntil:'domcontentloaded'});
await p.waitForTimeout(1200);
await p.screenshot({path:'shots/v0-loader.png'});
await p.waitForTimeout(4400);
await p.screenshot({path:'shots/v1-hello.png'});
...
await p.screenshot({path:'shots/v3-projects.png'});
const cards=await p.locator('.card').count();
await p.locator('.card').first().hover();
await p.waitForTimeout(1700);
await p.screenshot({path:'shots/v4-overlay.png'});
await b.close();
