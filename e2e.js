...
await p.goto('http://localhost:8092/',{waitUntil:'load'});
await p.waitForTimeout(2200);
await p.screenshot({path:'shots/t1-hello.png'});
await p.waitForTimeout(5000);
await p.screenshot({path:'shots/t2-name.png'});
for(let i=0;i<3;i++){await p.mouse.move(640,400);await p.mouse.wheel(0,150);await p.waitForTimeout(750);}
await p.waitForTimeout(1400);
await p.screenshot({path:'shots/t3-projects.png'});
const cards=await p.locator('.card').count();
await p.locator('.card').first().hover();
await p.waitForTimeout(1700);
await p.screenshot({path:'shots/t4-overlay.png'});
for(let i=0;i<8;i++){await p.mouse.wheel(0,320);await p.waitForTimeout(140);}
await p.waitForTimeout(1100);
await p.screenshot({path:'shots/t5-expanded.png'});
await b.close();
