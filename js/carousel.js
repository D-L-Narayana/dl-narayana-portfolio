...
function measure(){
fullH=Math.min(360,Math.max(96,window.innerHeight*0.264));
...
slide(false);}
function applyBg(first){
var p=P[idx],img=document.getElementById('cbgImg');
var doSwap=function(){
img.src=p.img;
document.getElementById('cbgColor').style.backgroundColor=p.accent;
document.getElementById('cbgMult').style.backgroundColor=p.accent;
gsap.fromTo(img,{scale:1.42},{scale:1.28,duration:5,ease:'none'});
gsap.to('#cbg',{opacity:1,duration:0.5});};
if(first){doSwap();}else{gsap.to('#cbg',{opacity:0,duration:0.28,onComplete:doSwap});}}
function setHead(){
...
gsap.fromTo(s,{opacity:0,y:6},{opacity:0.8,y:0,duration:0.45,delay:0.12+i*0.06});});}
function rail(){
...
slide(true);applyBg(false);setHead();rail();return true;}
function build(){
...
strip.appendChild(b);});
isBuilt=true;measure();applyBg(true);setHead();rail();
window.addEventListener('resize',function(){if(isBuilt)measure();});}
return{build:build,built:function(){return isBuilt;},showBg:function(){applyBg(true);},
